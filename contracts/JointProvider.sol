...
interface IRebalancer {
    ...
    function balanceOfLbp() external view returns (uint256);

    function looseBalanceA() external view returns (uint256);

    function looseBalanceB() external view returns (uint256);

    function pooledBalanceA() external view returns (uint256);

    function pooledBalanceB() external view returns (uint256);

    function pooledBalance(uint256 index) external view returns (uint256);

    function totalBalanceOf(IERC20 _token) external view returns (uint256);

    function currentWeightA() external view returns (uint256);

    function currentWeightB() external view returns (uint256);
...
contract JointProvider is BaseStrategy {
    ...
    function setRebalancer(address payable _rebalancer)
        external
        onlyGovernance
    {
    ...
    function estimatedTotalAssets() public view override returns (uint256) {
        return
            want.balanceOf(address(this)).add(rebalancer.totalBalanceOf(want));
    }
    ...
    {
        return
            super.harvestTrigger(callCostInWei) && rebalancer.shouldHarvest();
    }
        ...
        if (_debtOutstanding > 0) {
            if (vault.strategies(address(this)).debtRatio == 0) {
                ...
                _loss = _debtOutstanding > _debtPayment
                    ? _debtOutstanding.sub(_debtPayment)
                    : 0;
            } else {
        ...
        returns (uint256 _liquidatedAmount, uint256 _loss)
    {
        ...
        uint256 pooled = rebalancer.pooledBalance(rebalancer.tokenIndex(want));

        if (_amountNeeded > loose) {
            uint256 _amountNeededMore = _amountNeeded.sub(loose);
            if (_amountNeededMore >= pooled) {
                rebalancer.liquidateAllPositions(want, address(this));
            } else {
                rebalancer.liquidatePosition(
                    _amountNeededMore,
                    want,
                    address(this)
                );
