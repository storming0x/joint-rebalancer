...
interface ILiquidityBootstrappingPool {
    ...
    function balanceOf(address) external view returns (uint256 _amount);
    ...
    function getNormalizedWeights()
        external
        view
        returns (uint256[] calldata _normalizedWeights);

    function setSwapFeePercentage(uint256 _fee) external;

    function getSwapFeePercentage() external view returns (uint256 _fee);
...
contract Rebalancer {
    ...
    IWETH9 private constant weth =
        IWETH9(address(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2));
    ...
    modifier onlyAllowed() {
        require(
            msg.sender == address(providerA) ||
                ...
                msg.sender == providerA.getGovernance(),
            "!allowed"
        );
        _;
    }
    ...
    function initialize(
        ...
        require(
            address(providerA) == address(0x0) &&
                address(tokenA) == address(0x0),
            "Already initialized!"
        );
        require(
            address(providerB) == address(0x0) &&
    ...
    function _initialize(
        ...
        initialWeights[0] = uint256(0.5 * 1e18);
        initialWeights[1] = uint256(0.5 * 1e18);
    ...
    function name() public view returns (string[] memory) {
        ...
        names[1] = string(
            abi.encodePacked(
                ISymbol(address(tokenA)).symbol(),
                "-",
                ISymbol(address(tokenB)).symbol()
            )
        );
        return names;
    }
    ...
    function collectTradingFees() public onlyAllowed {
        uint256 debtA = providerA.totalDebt();
        uint256 debtB = providerB.totalDebt();

        if (debtA == 0 || debtB == 0) return;

        uint256 pooledA = pooledBalanceA();
        uint256 pooledB = pooledBalanceB();
        uint256 lbpTotal = balanceOfLbp();

        // there's profit
        if (pooledA >= debtA && pooledB >= debtB) {
            uint256 gainA = pooledA.sub(debtA);
            uint256 gainB = pooledB.sub(debtB);
            uint256 looseABefore = looseBalanceA();
            uint256 looseBBefore = looseBalanceB();
            ...
            if (gainA > 0) {
                tokenA.transfer(
                    address(providerA),
                    looseBalanceA().sub(looseABefore)
                );
            }
            ...
            if (gainB > 0) {
                tokenB.transfer(
                    address(providerB),
                    looseBalanceB().sub(looseBBefore)
                );
            }
        }
    }
    ...
    function sellRewards() public onlyAllowed {
        ...
        if (_rewards > 0) {
            uint256 rewardsA = _rewards.mul(currentWeightA()).div(1e18);
            uint256 rewardsB = _rewards.sub(rewardsA);
    ...
    function shouldHarvest() public view returns (bool _shouldHarvest) {
        uint256 debtA = providerA.totalDebt();
        uint256 debtB = providerB.totalDebt();
        uint256 totalA = totalBalanceOf(tokenA);
        uint256 totalB = totalBalanceOf(tokenB);
        return
            (totalA >= debtA && totalB > debtB) ||
            (totalA > debtA && totalB >= debtB);
    }
    ...
    function shouldTend() public view returns (bool _shouldTend) {
        // 18 == decimals of USD
        uint256 debtAUsd = _adjustDecimals(
            providerA.totalDebt().mul(providerA.getPriceFeed()).div(
                10**providerA.getPriceFeedDecimals()
            ),
            _decimals(tokenA),
            18
        );
        uint256 debtBUsd = _adjustDecimals(
            providerB.totalDebt().mul(providerB.getPriceFeed()).div(
                10**providerA.getPriceFeedDecimals()
            ),
            _decimals(tokenB),
            18
        ...
        uint256 idealAUsd = debtAUsd.add(debtBUsd).mul(currentWeightA()).div(
            1e18
        );
        uint256 idealBUsd = debtAUsd.add(debtBUsd).sub(idealAUsd);

        uint256 weight = debtAUsd.mul(1e18).div(debtTotalUsd);
        if (weight > 0.95 * 1e18 || weight < 0.05 * 1e18) {
            return true;
        }

        uint256 amountIn = _adjustDecimals(
            idealAUsd
                .sub(debtAUsd)
                .mul(10**providerA.getPriceFeedDecimals())
                .div(providerA.getPriceFeed()),
            18,
            _decimals(tokenA)
        );
        uint256 amountOutIfNoSlippage = _adjustDecimals(
            debtBUsd
                .sub(idealBUsd)
                .mul(10**providerB.getPriceFeedDecimals())
                .div(providerB.getPriceFeed()),
            18,
            _decimals(tokenB)
        );
        uint256 amountOut;
        ...
        if (idealAUsd > debtAUsd) {
            amountOut = BalancerMathLib.calcOutGivenIn(
                pooledBalanceA(),
                currentWeightA(),
                pooledBalanceB(),
                currentWeightB(),
        ...
        if (amountOut > amountOutIfNoSlippage) {
            uint256 slippage = amountOut
                .sub(amountOutIfNoSlippage)
                .mul(
                    10 **
                        (
                            idealAUsd > debtAUsd
    ...
    function adjustPosition() public onlyAllowed {
        ...
        tokenA.transferFrom(
            address(providerA),
            address(this),
            providerA.balanceOfWant()
        );
        tokenB.transferFrom(
            address(providerB),
            address(this),
            providerB.balanceOfWant()
        );
        ...
        // 18 == decimals of USD
        uint256 debtAUsd = _adjustDecimals(
            providerA.totalDebt().mul(providerA.getPriceFeed()),
            _decimals(tokenA),
            18
        );
        uint256 debtBUsd = _adjustDecimals(
            providerB.totalDebt().mul(providerB.getPriceFeed()),
            _decimals(tokenB),
            18
        );
        uint256 debtTotalUsd = debtAUsd.add(debtBUsd);
        ...
        newWeights[0] = Math.max(
            Math.min(debtAUsd.mul(1e18).div(debtTotalUsd), 0.9 * 1e18),
            0.1 * 1e18
        );
        newWeights[1] = 1e18 - newWeights[0];
        lbp.updateWeightsGradually(now, now, newWeights);
        bool atLimit = newWeights[0] == 0.9 * 1e18 ||
            newWeights[0] == 0.1 * 1e18;

        uint256 looseA = looseBalanceA();
        uint256 looseB = looseBalanceB();
        ...
        if (newWeights[0] == 0.1 * 1e18) {
            amountsIn[1] = _adjustDecimals(
                looseA.mul(24).mul(providerA.getPriceFeed()).div(
                    providerB.getPriceFeed()
                ),
                _decimals(tokenA),
                _decimals(tokenB)
            );
            ...
            amountsIn[0] = _adjustDecimals(
                looseB.mul(24).mul(providerB.getPriceFeed()).div(
                    providerA.getPriceFeed()
                ),
                _decimals(tokenB),
                _decimals(tokenA)
            );
        ...
        IBalancerVault.JoinPoolRequest memory request = IBalancerVault
            .JoinPoolRequest(assets, maxAmountsIn, userData, false);
    ...
    function liquidatePosition(
        ...
        toOnlyAllowed(_to)
        onlyAllowed
        returns (uint256 _liquidated, uint256 _short)
    {
        uint256 index = tokenIndex(_token);
        uint256 loose = _token.balanceOf(address(this));

        if (_amountNeeded > loose) {
            uint256 _pooled = pooledBalance(index);
            uint256 _amountNeededMore = Math.min(
                _amountNeeded.sub(loose),
                _pooled
            );
    ...
    function liquidateAllPositions(IERC20 _token, address _to)
        public
        toOnlyAllowed(_to)
        onlyAllowed
    ...
    function _swap(
        ...
        uint256 decIn = ERC20(_path[0]).decimals();
        uint256 decOut = ERC20(_path[_path.length - 1]).decimals();
        uint256 decDelta = decIn > decOut ? decIn.sub(decOut) : 0;
    ...
    function _getPath(IERC20 _in, IERC20 _out)
        internal
        pure
        returns (address[] memory _path)
    {
        bool isWeth = address(_in) == address(weth) ||
            address(_out) == address(weth);
    ...
    function setTendBuffer(uint256 _newBuffer) external onlyAuthorized {
    ...
    function ethToWant(address _want, uint256 _amtInWei)
        external
        view
        returns (uint256 _wantAmount)
    {
        if (_amtInWei > 0) {
    ...
    function pooledBalance(uint256 index) public view returns (uint256) {
        (, uint256[] memory balances, ) = bVault.getPoolTokens(lbp.getPoolId());
        return balances[index];
    }

    function totalBalanceOf(IERC20 _token) public view returns (uint256) {
        uint256 pooled = pooledBalance(tokenIndex(_token));
        uint256 loose = _token.balanceOf(address(this));
    ...
    function _decimals(IERC20 _token)
        internal
        view
    ...
    function tokenIndex(IERC20 _token)
        public
        view
        returns (uint256 _tokenIndex)
    {
